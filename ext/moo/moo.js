(function(root, factory) {
  ...
  function ruleOptions(name, obj) {
    ...
    if (options.keywords) {
      if(options.getType) {
        // An edit for the Keyman web-piler project; since we allow unusual identifiers, we
        // need to be able to overwrite a token's type.
        options.getTypeKeywords = keywordTransform(options.keywords);
        options.getTypeAlt = options.getType;
        options.getType = function(text) {
          var retVal = options.getTypeKeywords(text);

          // If the keywords block didn't change the token type, then run our custom chunk.
          if(typeof(retVal) == 'undefined') {
            return options.getTypeAlt(text);
          } else {
            return retVal;
          }
        }
